    ...
    if(isset($_REQUEST['type']) && $_POST['id']){
        ...
        switch($Case_){
            case 'active':
                ...
                $SQL = mysqli_fetch_assoc(mysqli_query($kunloc,"SELECT mode FROM botcmt WHERE id = '$i' AND username= '$username'"));
                if($SQL['mode'] == 'bat'){ 
                    $trangthai = 'tat'; 
                    $capnhat = mysqli_query($kunloc,"UPDATE botcmt SET mode = '$trangthai' WHERE id = '$i' ");
                    $JSON = array(
