...
class HelperImage
{
    const URL_IMG = URL_API_IMAGE;
    public function __construct()
    {

    }

    private function effect($type = 'text'){
...
662|frame hộp valentine|image';
    ...
    public function getUrlImage($text,$type='text'){
    
        try{
            $id= $this->effect($type);
            $url = self::URL_IMG.$id.'/param1/?param1='.$text;
            $curl = curl_init();
//echo $url;
...
curl_close($curl);
			...
			if(is_null($json)) return self::getUrlImage($text,$type);
