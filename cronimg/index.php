           ...
           if(preg_match_all('#ft_ent_identifier=(.+?)&#is',$login, $id_post_user)) {
            for($i=0; $i<1; $i++) { #count($id_post_user['1']) sll
                 ...
                 if(Loc($id_post,$token)){
                        $getinfo = getName($id_post,$token,$cookie);
...
function getName($id,$token,$cookie=''){
    ...
    $postService = new PostService($id,$cookie);
    $name=$postService->getNamePoster();
    if(is_null($name)) return null;
    return $name;
}
function getImg($text,$domain_url){
    require_once '../support/HelperImage.php';
    $helper = new HelperImage();
    $url= $helper->getUrlImage($text);
